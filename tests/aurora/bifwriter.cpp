...
/**
 * @file
 * Unit tests for our BIF file writer
 */
...
GTEST_TEST(BIFWriter, writeFile) {
	...
	Aurora::BIFWriter bif(1, writeStream);

	bif.add(stream, Aurora::kFileTypeTXT);

	EXPECT_EQ(bif.size(), 20 + 16 + kTextLength);

	Common::MemoryReadStream *bifStream = new Common::MemoryReadStream(writeStream.getData(), writeStream.size(), true);
	const Aurora::BIFFile bifFile(bifStream);
...
GTEST_TEST(BIFWriter, writeMultipleFiles) {
	...
	Common::MemoryWriteStreamDynamic writeStream(false);
	Aurora::BIFWriter bif(3, writeStream);

	bif.add(textStream, Aurora::kFileTypeTXT);
	bif.add(textStream, Aurora::kFileTypeTXT);
	bif.add(imageStream, Aurora::kFileTypeBMP);

	EXPECT_EQ(bif.size(), 20 + (3 * 16) + (2 * kTextLength) + kLogoDataSize);
	...
	EXPECT_EQ(bifFile.getInternalResourceCount(), 3);
	EXPECT_EQ(bifFile.getResourceSize(0), kTextLength);
	EXPECT_EQ(bifFile.getResourceSize(1), kTextLength);
	EXPECT_EQ(bifFile.getResourceSize(2), kLogoDataSize);

	Common::SeekableReadStream *dataStream = bifFile.getResource(0);
	...
	dataStream = bifFile.getResource(1);
	...
	dataStream = bifFile.getResource(2);
