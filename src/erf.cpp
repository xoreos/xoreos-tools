...
int main(int argc, char** argv) {
	...
	try {
		...
		if (!writeFile.isOpen())
			throw Common::Exception("Could not open output archive file");

		Aurora::ERFWriter erfWriter(id, files.size(), writeFile);
		for (std::set<Common::UString>::const_iterator iter = files.begin(); iter != files.end(); ++iter) {
			std::printf("Packing: %s ... ", iter->c_str());

			Common::UString file = *iter;
			Common::ReadFile fileStream(file);

			if (!fileStream.isOpen())
				throw Common::Exception("Could not open %s", file.c_str());
...
bool parseCommandLine(const std::vector<Common::UString> &argv, int &returnValue,
	...
	parser.addOption("erf", "Set ERF as archive id",
	                 kContinueParsing,
